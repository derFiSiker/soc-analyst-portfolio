...
const platformImage = "/manus-storage/project-siem_2402fb1c.jpg";
const stackIcons = [Terminal, ShieldCheck, Server, Server, Network, Server];

function SectionMarker({ number, label }: { number: string; label: string }) {
  return <div className="section-marker" aria-label={`Section ${number}: ${label}`}><span>{number}</span><img className="section-node" src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><i /><strong>{label}</strong></div>;
}
  ...
  return (
    <div className="signal-site" lang={language}>
      <header className="site-header">
        <a className="brand" href="#top" aria-label={copy.brandBackToTop}><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span><b>Marco</b><em>Lütkemüller</em></span></a>
      ...
      <main id="top">
        <section className="hero" aria-labelledby="hero-title">
          ...
          <div className="telemetry-card"><div className="telemetry-head"><span>{copy.hero.telemetry.title}</span><span className="telemetry-symbol"><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><CircleDot size={15} /></span></div><div className="telemetry-grid"><div><span>{copy.hero.telemetry.source}</span><b>{copy.hero.telemetry.sourceValue}</b></div><div><span>{copy.hero.telemetry.siem}</span><b>{copy.hero.telemetry.siemValue}</b></div><div><span>{copy.hero.telemetry.lab}</span><b>{copy.hero.telemetry.labValue}</b></div><div><span>{copy.hero.telemetry.mode}</span><b>{copy.hero.telemetry.modeValue}</b></div></div></div>
      ...
      <footer id="contact" className="site-footer"><div className="footer-grid"><div className="footer-intro"><div className="eyebrow"><span className="live-dot" /> {copy.contact.eyebrow}</div><h2>{copy.contact.heading[0]}<br /><i>{copy.contact.heading[1]}</i></h2><p>{copy.contact.body}</p><div className="footer-location"><MapPin size={15} />{copy.contact.location}</div><div className="footer-links"><a href={githubUrl} target="_blank" rel="noreferrer"><Github size={17} />{copy.contact.github}</a><a href={linkedinUrl} target="_blank" rel="noreferrer"><Linkedin size={17} />{copy.contact.linkedin}</a><a href="/manus-storage/Marco_Luetkemuller_Portfolio_CV_67f40ebf.pdf" download><Download size={17} />{copy.contact.cv}</a></div></div><form className="contact-form" onSubmit={submitContact}><div className="form-heading"><Mail size={18} /><span>{copy.contact.formTitle}</span></div><label>{copy.contact.name}<input required name="name" placeholder={copy.contact.namePlaceholder} /></label><label>{copy.contact.email}<input required type="email" name="email" placeholder={copy.contact.emailPlaceholder} /></label><label>{copy.contact.message}<textarea required name="message" rows={4} placeholder={copy.contact.messagePlaceholder} /></label><input type="hidden" name="_subject" value="Portfolio contact request" /><button className="button button-primary form-submit" type="submit" disabled={formStatus === "sending"}>{formStatus === "sending" ? copy.contact.sending : copy.contact.submit} <Send size={16} /></button><p className="form-privacy">{copy.contact.privacy}</p>{formNotice && <p className={`form-note form-${formStatus}`}>{formNotice}</p>}</form></div><div className="footer-bottom"><span>© 2026 Marco Lütkemüller</span><span><Command size={13} /> {copy.footer.trace}</span><a href="#top">{copy.footer.back} <ArrowUpRight size={13} /></a></div></footer>
    </div>
  );
}
