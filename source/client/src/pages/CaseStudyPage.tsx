...
export default function CaseStudyPage({ caseId }: { caseId: string }) {
  ...
  if (!caseStudy) {
    return <div className="signal-site case-page"><header className="site-header"><a className="brand" href="../../"><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span><b>Marco</b><em>Lütkemüller</em></span></a></header><main className="case-missing"><p className="eyebrow"><span className="live-dot" /> 404 / CASE STUDY</p><h1>{labels.unavailable}</h1><p>{labels.unavailableCopy}</p><a className="button button-primary" href="../../">{labels.home} <ArrowLeft size={17} /></a></main></div>;
  }

  return (
    <div className="signal-site case-page" lang={language}>
      <div className={`case-loader ${caseLoading ? "is-visible" : ""}`} aria-live="polite" aria-hidden={!caseLoading}><div><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span>{labels.loading}</span><i /></div></div>
      <header className="site-header case-header">
        <a className="brand" href="../../" aria-label="Marco Lütkemüller home"><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span><b>Marco</b><em>Lütkemüller</em></span></a>
      ...
      <main>
        <section className="case-hero">
          ...
          <div className="case-hero-grid"><div><p className="eyebrow"><span className="live-dot" /> {caseStudy.kind}</p><h1>{caseStudy.title}</h1><p className="case-lead">{caseStudy.lead}</p><div className="case-hero-actions"><a className="button button-primary case-return-button" href="../../#projects"><ArrowLeft size={17} /> {labels.back}</a>{hasRepository && <a className="button button-quiet" href={repositoryUrl(caseId)} target="_blank" rel="noreferrer"><Github size={17} /> {labels.repository} <ExternalLink size={15} /></a>}</div></div><aside className="case-identity"><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span>CASE / {caseStudy.id.toUpperCase()}</span><i /><b>01</b></aside></div>
        </section>
        ...
        <section className="artifact-section"><div className="artifact-head"><p className="eyebrow"><span className="live-dot" /> {labels.artifacts}</p><h2>{labels.artifacts}<br /><i>in Vorbereitung.</i></h2></div><div className="artifact-grid"><article className="artifact-slot screenshot-slot"><div className="artifact-top"><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span>01 / {labels.screenshot}</span><i /></div><div className="artifact-screen"><header><span>CAPTURE // LAB VIEW</span><b>PENDING</b></header><span className="artifact-cross" /><span className="artifact-grid-lines" /><footer><span>ARCHITECTURE</span><span>ALERT VIEW</span></footer></div><p>{labels.screenshotHint}</p></article><article className="artifact-slot detection-slot"><div className="artifact-top"><img src="/manus-storage/marco-signal-logo_ff8cb29d.png" alt="" /><span>02 / {labels.detection}</span><i /></div><div className="artifact-code"><header><span>RULE // Triage</span><b>PLACEHOLDER</b></header><span>rule://{caseStudy.id}</span><i /><i /><i /><i /><footer>log source · severity · investigation step</footer></div><p>{labels.detectionHint}</p></article></div></section>
